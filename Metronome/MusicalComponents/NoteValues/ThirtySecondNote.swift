...
struct ThirtySecondNote: MusicalComponent, View {
    let width: CGFloat?
    let height: CGFloat?
    ...
    var body: some View {
        GeometryReader{ geometry in
            HStack(alignment: .top, spacing: 0){
                ...
                Image("ThirtySecondFlag")
                    .resizable()
                    .aspectRatio(contentMode: .fit)
                    .frame(height: geometry.size.height - geometry.size.height/3.8150/4) // height - head_size/4
            }/*.aspectRatio((1+1/1.6)/3.14, contentMode: .fit)  // needed?*/
        }.frame(width: width, height: height)
    }
}

struct ThirtySecondNote_Previews: PreviewProvider {
    static var previews: some View {
        ThirtySecondNote(relativeHeight: 100)
    }
}
